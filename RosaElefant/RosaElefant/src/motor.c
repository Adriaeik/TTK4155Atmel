...
void motor_control_velocity(void) {
	...
	double midDutyMs = 0.0; // Midtverdi, som er 0 % n�r motoren st�r stille

	int x_pos = board.JoyXposCal;  // Les joystick posisjon

	// Kalkuler ms-verdi (duty cycle) basert p� joystickens posisjon
	if (abs(x_pos) < 5) {
	...
	// Sett motor retning basert p� fortegnet til joystick-verdien
	if (x_pos > 0)PIOC->PIO_SODR = PHASE_PIN;  // Sett pin 0 h�g (positiv retning)} 
	else if (x_pos < 0)PIOC->PIO_CODR = PHASE_PIN;  // Sett pin 0 l�g (negativ retning)

	// Optional: printf for � debug duty cycle og retning
...
int normalize_pos_encoder(int prev_pos) {
	...
	// S�rg for at posisjonen ikkje er negativ
...
int normalize_pos_ref(int prev_ref) {
	...
	return (int)((double)slider_pos / SLIDER_RANGE* a*PARAM_SCALE + prev_ref * b)/10; //IKKJE 1000 for � fjerne 10
}
...
void motor_control_PID(void) {
	...
	if (reset_PID_flag == 0) {
		if (inverted_controll) {
			// Inverter referansen basert p� SLIDER_RANGE
	...
	// Sett motor retning basert p� fortegnet til avviket

	if (error > 0) {
		if (abs(error)> error_hyst){
			PIOC->PIO_SODR = PHASE_PIN;  // Sett pin h�g (positiv retning) mot h�yre?
			}
		} else if (error < 0) {
		if (abs(error)> error_hyst){
			PIOC->PIO_CODR = PHASE_PIN;  // Sett pin l�g (negativ retning)
		}
	}
	...
	//VIKTIG med abs verdi d� u m� vere eit tall mellom 0 og 1
	long long int u = clamp(abs(actuation), 0, PARAM_SCALE);	// Normaliser og klamp p�draget
...
// Funksjon for � oppdatere motor PWM
...
void reset_pid(void) {
	...
	integral = 0;
	derivat = 0;
	uint32_t timeout = 1000; // Tidsavbrudd for � unng� evig l�kke
	ref = 0;
...
int calibrate_motor_pos(void) {
	...
	const int stable_threshold = 15;  // Antal gonger posisjonen m� vere uendra
	...
	while (stable_count < stable_threshold) {
		...
		PIOC->PIO_SODR = PHASE_PIN;  // Sett pin l�g (negativ retning)
		// Sett motor retning basert p� fortegnet til avviket
		//error = -1;
		//if (error > 0) {
			//PIOC->PIO_SODR = PHASE_PIN;  // Sett pin h�g (positiv retning) mot h�yre?
